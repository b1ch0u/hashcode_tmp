import random
from math import log, sqrt

class Node:

	def __init__(self, state):
		self.state = state
		self.win_value = 0
		self.policy_value = None
		self.visits = 0
		self.parent = None
		self.children = []
		self.expanded = False
		self.player_number = None
		self.discovery_factor = 0.35
	
	def print_recursive(self, indent_lvl=0):
		print('    ' * indent_lvl + f'| - {id(self)} : visited {self.visits} times, score {self.state.score}')
		for child in self.children:
			child.print_recursive(indent_lvl+1)
		
	def compute_sub_height(self):
		if not self.expanded:
			return 1
		return 1 + max(child.compute_sub_height()
						for child in self.children)

	def update_win_value(self, value):
		self.win_value += value
		self.visits += 1
		if self.parent:
			self.parent.update_win_value(value)

	def update_policy_value(self, value):
		self.policy_value = value

	def add_child(self, child):
		self.children.append(child)
		child.parent = self

	def add_children(self, children):
		for child in children:
			self.add_child(child)

	def get_preferred_child(self, root_node):
		# print('GET PREFERRED CHIDL')
		best_children = []
		best_score = float('-inf')
		scores = []
		for child in self.children:
			score = child.get_score(root_node)
			scores.append(score)
			if score >= best_score:
				best_score = score
				best_children.append(child)
		# print(id(self), 'scores', scores, id(best_children[0]))
		chosen_child = random.choice(best_children)
		# print('chosen child', (chosen_child))
		return chosen_child

	def get_score(self, root_node):
		discovery_operand = self.discovery_factor * (self.policy_value or 1) * sqrt(log(self.parent.visits) / (self.visits or 1))
		win_operand = self.win_value / (self.visits or 1)
		self.score = win_operand + discovery_operand
		return self.score

	def is_scorable(self):
		return self.visits or self.policy_value != None